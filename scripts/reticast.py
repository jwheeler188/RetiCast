...
PAGE_PATH = "/page/reticast.mu"   # where the weather page is served (used for alert links)
...
def build_page(meta, data, stale=False):
    ...
    if alerts:
        ...
        for a in alerts:
            ...
            if a.get("headline"):
                out.append(esc(trim(a["headline"], 0)[0]))
            cut = False
            if a.get("description"):
                text, cut = trim(a["description"], MAX_ALERT_CHARS)
                out.append(esc(text))
            if a.get("instruction"):
                text, cut_i = trim(a["instruction"], MAX_ALERT_CHARS)
                out.append("`!What to do:`! " + esc(text))
                cut = cut or cut_i
            if cut:
                out.append(f"`_`[Read full alert`:{PAGE_PATH}`alert={alert_key(a)}]`_")
            out.append("")
...
def build_alert(meta, data, key):
    tz = meta["timezone"]
    back = f"`_`[Back to weather`:{PAGE_PATH}]`_"
    for a in data.get("alerts", []):
        if alert_key(a) != key:
            continue
        end = parse_time(a.get("ends"))
        out = [f"{alert_color(a['event'])}`!{esc(a['event'].upper())}`!`f"
               + (esc(f"  until {fmt_local(end, '%I:%M%p %a %m/%d', tz)}") if end else ""),
               ""]
        if a.get("headline"):
            out += [esc(trim(a["headline"], 0)[0]), ""]
        if a.get("area"):
            out += ["`!Areas:`! " + esc(a["area"]), ""]
        if a.get("description"):
            out += [esc(trim(a["description"], 0)[0]), ""]
        if a.get("instruction"):
            out += ["`!What to do:`!", esc(trim(a["instruction"], 0)[0]), ""]
        if a.get("sender"):
            out += [f"`F888Issued by {esc(a['sender'])}`f", ""]
        out.append(back)
        return "\n".join(out)
    return "That alert is no longer active.\n\n" + back
...
def get_alert_micron(key):
    """Full text of one alert, for the 'Read full alert' link."""
    try:
        meta, data, _ = get_data()
        return build_alert(meta, data, key)
    except Exception as e:
        return f"Alert unavailable ({type(e).__name__})\n\n`_`[Back to weather`:{PAGE_PATH}]`_"
