...
- Full weather page: every active alert with its details and safety instructions, current conditions including feels-like temperature, wind, pressure, and visibility, and a 7-day forecast.
- US or metric units.
...
The demo home page already includes this link in its menu.
...
print("`F0ff`!Weather:`! " + weather + "`f")
```

For the full page content instead of one line, use `get_weather_micron()`. The `try` block makes sure a weather problem can never stop the rest of the page from loading.
